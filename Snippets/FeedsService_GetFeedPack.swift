...
func sample(
  ...
  let response = try await client.getFeedPack(
    request: GetFeedPackRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/instances/\(instanceId)/feedPacks/\(feedPackId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        instanceId: "[placeholder]", feedPackId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
