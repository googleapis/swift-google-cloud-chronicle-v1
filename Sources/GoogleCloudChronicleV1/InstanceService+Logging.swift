...
extension Clients {
  final class InstanceServiceLogging: InstanceServiceStub {
    let inner: any InstanceServiceStub
    let logger: Logger

    public init(_ inner: any InstanceServiceStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-chronicle-v1"
      logger[metadataKey: "gcp.client.service"] = "chronicle"
      logger[metadataKey: "gcp.experimental.swift.client"] = "InstanceService"
      self.inner = inner
      self.logger = logger
    }
