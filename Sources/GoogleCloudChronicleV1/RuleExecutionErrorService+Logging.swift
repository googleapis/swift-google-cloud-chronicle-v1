...
extension Clients {
  final class RuleExecutionErrorServiceLogging: RuleExecutionErrorServiceStub {
    let inner: any RuleExecutionErrorServiceStub
    let logger: Logger

    public init(_ inner: any RuleExecutionErrorServiceStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-chronicle-v1"
      logger[metadataKey: "gcp.client.service"] = "chronicle"
      logger[metadataKey: "gcp.experimental.swift.client"] = "RuleExecutionErrorService"
      self.inner = inner
      self.logger = logger
    }
    ...
    public func listRuleExecutionErrors(
      ...
      try await self._intercept(
        request: request,
        options: options,
        name: "listRuleExecutionErrors",
        action: {
          (r: ListRuleExecutionErrorsRequest, o: GoogleCloudGax.RequestOptions) async throws
            -> GoogleCloudChronicleV1.ListRuleExecutionErrorsResponse
          in
          return try await self.inner.listRuleExecutionErrors(request: r, options: o)
        })
    }
