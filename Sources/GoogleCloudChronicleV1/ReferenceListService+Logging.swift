...
extension Clients {
  final class ReferenceListServiceLogging: ReferenceListServiceStub {
    let inner: any ReferenceListServiceStub
    let logger: Logger

    public init(_ inner: any ReferenceListServiceStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-chronicle-v1"
      logger[metadataKey: "gcp.client.service"] = "chronicle"
      logger[metadataKey: "gcp.experimental.swift.client"] = "ReferenceListService"
      self.inner = inner
      self.logger = logger
    }
